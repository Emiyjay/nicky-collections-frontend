          ...
          <div className="relative mx-auto grid min-h-[calc(100vh-72px)] max-w-7xl items-center gap-12 px-6 py-20 md:px-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-20 lg:py-16">
            <motion.div initial="hidden" animate="show" variants={reveal} className="max-w-2xl">
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <Link href="/shop" className="btn-primary inline-flex items-center gap-2">Explore collection <FiArrowRight size={15} /></Link>
                <a href={`mailto:${CONTACT_EMAIL}`} className="btn-outline inline-flex items-center gap-2"><FiMail size={15} /> Order by Email</a>
              </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.97, x: 25 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.15 }} className="relative mx-auto w-full max-w-[590px]">
              <div className="relative aspect-[4/5] overflow-hidden border border-white/10 bg-brand-card">
                {heroImage ? <Image src={heroImage} alt={heroProduct?.images?.[0]?.alt || heroProduct?.name || 'Nicky Collections fashion'} fill priority sizes="(max-width: 1023px) 90vw, 48vw" className="object-cover" /> : <div className="absolute inset-0 flex items-center justify-center"><span className="font-display text-[8rem] font-light tracking-tighter text-white/10 md:text-[12rem]">NC</span></div>}
                ...
                <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-5"><div><p className="font-body text-[9px] font-semibold uppercase tracking-[0.22em] text-brand-pink">The edit</p><p className="mt-1 max-w-xs font-display text-2xl leading-none text-white md:text-3xl">{heroProduct?.name || 'Your next statement piece'}</p></div>{heroProduct && <Link href={`/product/${heroProduct.slug || heroProduct._id}`} aria-label={`Shop ${heroProduct.name}`} className="flex h-11 w-11 shrink-0 items-center justify-center bg-white text-brand-dark transition-transform hover:scale-105"><FiArrowUpRight size={18} /></Link>}</div>
              </div>
              <div className="absolute -bottom-4 -left-4 hidden border border-brand-gold/30 bg-brand-dark/90 px-4 py-3 backdrop-blur-md sm:block"><p className="font-body text-[9px] uppercase tracking-[0.2em] text-brand-gold">New drops</p><p className="mt-1 font-body text-xs text-brand-light">Every week on TikTok</p></div>
            </motion.div>
          </div>
