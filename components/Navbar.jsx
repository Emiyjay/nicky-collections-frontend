  ...
  return (
    <>
      ...
      <nav
        ...
        className={`fixed top-8 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-brand-dark/95 backdrop-blur-md border-b border-white/5' : 'bg-transparent'}`}
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between h-16">
            <Link href="/" className="flex flex-col leading-none" aria-label="Nicky Collections home">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              {user ? (
                <>
                  <Link href="/dashboard" aria-label="Wishlist and dashboard" className="p-2 text-brand-gray hover:text-brand-light transition-colors">
                    <FiHeart size={18} aria-hidden="true" />
                  </Link>
                  <Link href={user.role === 'admin' ? '/admin' : '/dashboard'} aria-label="Account" className="p-2 text-brand-gray hover:text-brand-light transition-colors">
                    <span className="font-body text-xs tracking-widest uppercase">Account</span>
                  </Link>
                </>
              ) : (
                <Link href="/auth/login" aria-label="Log in" className="p-2 text-brand-gray hover:text-brand-light transition-colors">
                  <span className="font-body text-xs tracking-widest uppercase">Log in</span>
                </Link>
              )}

              <button type="button" onClick={toggleDarkMode} aria-label={darkMode ? 'Switch to light mode' : 'Switch to dark mode'} className="p-2 text-brand-gray hover:text-brand-gold transition-colors hidden md:block">
                {darkMode ? <FiSun size={16} aria-hidden="true" /> : <FiMoon size={16} aria-hidden="true" />}
              </button>

              <button type="button" onClick={() => setMenuOpen(!menuOpen)} aria-expanded={menuOpen} aria-controls="mobile-navigation" aria-label={menuOpen ? 'Close navigation menu' : 'Open navigation menu'} className="p-2 md:hidden text-brand-light">
