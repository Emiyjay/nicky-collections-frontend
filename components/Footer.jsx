      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="md:col-span-1">
            ...
            <p className="font-body text-sm text-brand-gray leading-relaxed mb-6">Fashion-forward pieces promoted on TikTok. Order via WhatsApp for fast, personal service.</p>
            <div className="flex gap-4"><a href={TIKTOK_URL} target="_blank" rel="noreferrer" className="w-10 h-10 border border-white/10 flex items-center justify-center text-brand-gray hover:text-brand-pink hover:border-brand-pink transition-all"><FaTiktok size={16} /></a><a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" className="w-10 h-10 border border-white/10 flex items-center justify-center text-brand-gray hover:text-green-400 hover:border-green-400 transition-all"><FaWhatsapp size={16} /></a><a href="https://instagram.com/nickycollections" target="_blank" rel="noreferrer" className="w-10 h-10 border border-white/10 flex items-center justify-center text-brand-gray hover:text-pink-400 hover:border-pink-400 transition-all"><FaInstagram size={16} /></a></div>
          </div>
          ...
          <div><h4 className="label-tag text-brand-light mb-6">Help</h4><ul className="space-y-3">
            {[{ label: 'About Us', href: '/about' }, { label: 'Contact', href: '/contact' }, { label: 'Order on WhatsApp', href: WHATSAPP_LINK, external: true }, { label: 'TikTok Shop', href: TIKTOK_URL, external: true }].map(item => <li key={item.label}>{item.external ? <a href={item.href} target="_blank" rel="noreferrer" className="font-body text-sm text-brand-gray hover:text-brand-light transition-colors">{item.label}</a> : <Link href={item.href} className="font-body text-sm text-brand-gray hover:text-brand-light transition-colors">{item.label}</Link>}</li>)}
          </ul></div>

          <div><h4 className="label-tag text-brand-light mb-6">Contact Us</h4><div className="space-y-4">
            <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm text-brand-gray hover:text-green-400 transition-colors"><FaWhatsapp size={16} className="text-green-400" /><span>{DISPLAY_PHONE_NUMBER}</span></a>
