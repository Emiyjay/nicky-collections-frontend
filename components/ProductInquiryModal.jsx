...
export default function ProductInquiryModal() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      if (event.key === 'Escape' && status !== 'sending') setRequest(null);
    };
  ...
  const submit = async (event) => {
    ...
    const payload = {
      ...
      price,
      userName: clean(form.name) || null,
      customerEmail: clean(form.email) || null,
      customerPhone: clean(form.phone) || null,
      color: selection.color || null,
      size: selection.size || null,
      quantity,
      deliveryLocation: clean(form.location) || null,
    ...
    try {
      ...
      console.warn('Product inquiry failed; opening email fallback.', error);
    }
      ...
      <section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="product-inquiry-title">
        ...
        {status === 'success' ? (
          <div className={styles.success}>
            ...
            <h2 id="product-inquiry-title">We have your request.</h2>
            <p>Nicky Collections has received your product inquiry. We will confirm availability and the next steps by email.</p>
            {reference && <p className={styles.reference}>Reference: <strong>{reference}</strong></p>}
            <button className={styles.primary} type="button" onClick={() => setRequest(null)}>Done</button>
          </div>
        ) : status === 'fallback' ? (
          <div className={styles.success}>
            <div className={styles.successIcon}><FiMail size={28} /></div>
            <p className={styles.eyebrow}>Email opened</p>
            <h2 id="product-inquiry-title">Finish your inquiry by email.</h2>
            <p>The direct service was unavailable, so your device&apos;s email composer has been prepared with the product and order details.</p>
            <button className={styles.primary} type="button" onClick={() => setRequest(null)}>Done</button>
          </div>
        ) : (
          <form onSubmit={submit}>
            <div className={styles.header}>
              <p className={styles.eyebrow}>Private order inquiry</p>
              <h2 id="product-inquiry-title">Let&apos;s arrange your order.</h2>
              <p>Tell us where to reach you and what you need. Your request is sent securely to Nicky Collections for confirmation.</p>
            </div>

            <div className={styles.product}>
              {product.images?.[0]?.url && <img src={product.images[0].url} alt="" />}
              <div><strong>{productLabel}</strong><span>{Number.isFinite(price) ? `$${price.toFixed(2)}` : 'Price on request'}</span>{(selection.color || selection.size) && <small>{[selection.color, selection.size].filter(Boolean).join(' · ')}</small>}</div>
            </div>
            ...
            <div className={styles.grid}>
              ...
              <label>Quantity<input required type="number" min="1" max="99" value={form.quantity} onChange={update('quantity')} inputMode="numeric" /></label>
            </div>
            ...
            <div className={styles.actions}>
              ...
              <button className={styles.primary} type="submit" disabled={status === 'sending'}>{status === 'sending' ? 'Sending…' : <><FiMail size={17} /> Send inquiry</>}</button>
            </div>
