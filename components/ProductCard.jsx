...
export default function ProductCard({ product, index = 0 }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      transition={{ delay: index * 0.08, duration: 0.5 }} className="card-product group">
      <Link href={productPath}>
        <div className="relative overflow-hidden bg-brand-muted aspect-[3/4] img-zoom">
          <Image src={mainImage} alt={product.name} fill className="object-cover transition-all duration-700"
            onMouseEnter={() => secondImage && setImgIdx(1)} onMouseLeave={() => setImgIdx(0)} />
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {product.isNewArrival && <span className="bg-brand-pink text-white text-[10px] tracking-widest uppercase px-2 py-1 font-body font-semibold">New</span>}
            {product.isFeatured && <span className="bg-brand-gold text-brand-dark text-[10px] tracking-widest uppercase px-2 py-1 font-body font-semibold">Featured</span>}
          </div>
        ...
        <div className="p-4">
          ...
          <h3 className="font-body text-sm text-brand-light font-medium leading-snug mb-2 line-clamp-2 group-hover:text-white transition-colors">{product.name}</h3>
          {product.numReviews > 0 && <div className="flex items-center gap-1 mb-2">
            {[...Array(5)].map((_, i) => <FiStar key={i} size={10} fill={i < Math.round(product.rating) ? '#D4A843' : 'none'} className={i < Math.round(product.rating) ? 'text-brand-gold' : 'text-brand-gray'} />)}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {product.comparePrice && product.comparePrice > product.price && <span className="font-body text-xs text-brand-gray line-through">${product.comparePrice.toFixed(2)}</span>}
            </div>
